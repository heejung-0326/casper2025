...
#subject,#name,#email{padding:5px 10px;
width: 230px;}
legend{
  text-align:center;
  font-size: 20px;
...
#btn_g > input{width: 150px;
  color:#B0B3C3;
}
#btn_g > input:last-child{
  ...
  <section>
    ...
    <fieldset>
      ...
      <p class="field">
        ...
        <input type="text" name="name" id="name" placeholder="이름을 입력하세요" maxlength="5" required>
      </p>
      <p class="field">
      ...
      <p class="field">
        ...
        <select name="mb_type" id="mb_type">
          ...
          <option value="구매 혜택/이벤트">구매 혜택/이벤트</option>
      ...
      <p class="field">
        ...
        <div>
          (<input type="text" name="txtLen" size="3" maxlength="3" value="125" readonly class="count">/500)글자가 남았습니다.
        </div>
      </p>

      <textarea name="msg" rows="3" id="msg" onkeydown="txtCount(this.form.msg,this.form.txtLen, 125);" onkeyup="txtCount(this.form.msg, this.form.txtLen, 125);"></textarea>
      <div class="agree">
...
<script>
  ...
  function form_check(){ // submit 최종 폼 체크
    if(!document.getElementById('mb_type').value){ //문의 유형을 선택하지 않은 경우라면 
      alert("문의유형을 선택해야 합니다."); //메세지 띄우고
      mb_type.focus(); //다시 문의 유형 폼으로 초점을 맞춘다.
      return false;
    }
    //2번째 라디오 버튼 (동의안함)을 선택안한 경우라면
  ...
  //글자수 체크하는 함수
  //텍스트 박스에 키보드 키를 누르고 글자를 입력하면 아래 txtCount함수가 호출되어 실행 되도록한다.
  //3개의 변수값을 전달받아 함수에 대입(textarea이름, input박스의 이름, 최대 글자수1000)
  function txtCount(msg,txtLen, maxlimit){
    ...
    if(msg.value.length>maxlimit){
      ...
      //125글자에서 글자개수를 뺀 값을 변수에 넣어서 출력한다.
